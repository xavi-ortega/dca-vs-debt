...
Backtesting toolkit to compare asset-backed debt strategies vs. dollar-cost averaging (DCA) with realistic fees, loan dynamics, and liquidation risk. The codebase is a monorepo with a reusable core library, a CLI, and a web dashboard (preview at https://dca-vs-debt.vercel.app/).

> Note: Roughly 99% of the code was generated by Codex (human input was mostly prompting, reviewing, and tweaking).

## Packages
- `packages/core`: Engine for simulations, fee models, and report builders. Exported as `@dca-vs-debt/core`.
...
- Dashboard: combined asset value charts, liquidation risk markers, pie/heatmap comparisons, and configurable inputs.
...
## Deployment
- Web preview: https://dca-vs-debt.vercel.app/ (built with Vercel)
- Recommended Vercel setup: root directory `.`, install `npm install`, build `npm run build`, output `packages/web/dist`.

## Data
Sample asset OHLC datasets live under `packages/web/public/datasets/`; CSVs are expected to include ISO dates and USD prices.
